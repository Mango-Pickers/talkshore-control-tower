...
function Users() {
  const { data, isLoading } = useTable<any>("profiles", {
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Every traveler aboard TalkShore."
      />
      <DataTable
        rows={data}
        loading={isLoading}
        columns={[
          ...
          },
          {
            ...
            label: "Role",
            render: (u: any) => (
              <Badge tone={u.role === "guide" ? "gold" : "muted"}>
                {u.role ?? "learner"}
              </Badge>
            ),
          },
          ...
          },
          {
            ...
            render: (u: any) => (
              <button
                onClick={() =>
                  mut.mutate({ id: u.id, patch: { banned: !u.banned } })
                }
