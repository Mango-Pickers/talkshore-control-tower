...
function Videos() {
  const { data, isLoading } = useTable<any>("videos", { order: { column: "id" } });
  return (
    <div>
      <PageHeader eyebrow="Media" title="Video library" description="Every clip powering Voyage Prep and Live Shores." />
      {isLoading ? (
        <div className="ts-card p-10 grid place-items-center"><Loader2 className="size-5 animate-spin text-gold" /></div>
      ) : data?.length === 0 ? (
        <div className="ts-card p-10 text-center text-sm text-muted-foreground">No videos uploaded yet.</div>
      ) : (
            ...
            <motion.div key={v.id}
              ...
              <div className="aspect-video bg-elevated relative overflow-hidden">
                {v.thumbnail_url ? (
                  ...
                  <div className="w-full h-full grid place-items-center text-muted-foreground"><Video className="size-8" /></div>
                )}
              </div>
              <div className="p-4">
