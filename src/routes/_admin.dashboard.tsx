  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div
          className={`size-10 rounded-xl grid place-items-center ${accent ?? "bg-elevated text-gold"}`}
        >
          {icon}
        </div>
      </div>
...
function Dashboard() {
  ...
  const chart = Array.from({ length: 12 }).map((_, i) => ({
    ...
    sessions: Math.round(
      ((liveSessions.data ?? 0) + i * 3) * (0.8 + Math.sin(i) * 0.2)
    ),
  }));
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <Stat
          label="Live shores"
          value={liveSessions.data ?? "—"}
          icon={<Waves className="size-5" />}
          delay={0.2}
        />
        <Stat
          label="Pending reports"
          value={pendingReports.data ?? "—"}
        ...
        <Stat
          ...
          accent="bg-[color:oklch(0.78_0.14_175)]/15 text-[oklch(0.78_0.14_175)]"
          delay={0.4}
        />
      </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chart}>
                ...
                <Tooltip
                  contentStyle={{
                    background: "oklch(0.27 0.055 260)",
